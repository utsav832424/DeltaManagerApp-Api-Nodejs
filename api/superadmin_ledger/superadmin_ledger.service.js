...
module.exports = {
  ...
  getAll: (data, callBack) => {
    ...
    if (data.year != "") {
      ...
      whereUid += `YEAR(s.date) = ${data.year}`;
    }
    ...
    if (data.user_id != "") {
      ...
      whereAid += `a.id=${data.user_id}`;
      whereUid += ` AND u.parent_id=${data.user_id}`;
    }
    ...
    pool.query(
      `select (SELECT  IFNULL(sum(s.amount), 0)as total_sum FROM salary s JOIN userlogin u ON u.code = s.code WHERE ${whereUid})as total_salary,IFNULL(ROUND(sum(s.sheet)), 0)AS total_admin, ROUND((IFNULL(ROUND(sum(s.sheet)), 0) * (SELECT admin_sharing FROM userlogin a WHERE ${whereAid})) /100)as total_sum,(SELECT admin_sharing FROM userlogin a WHERE ${whereAid})as admin_sharing,(IFNULL((SELECT  IFNULL(sum(s.amount), 0)as total_sum FROM salary s JOIN userlogin u ON u.code = s.code WHERE ${whereUid}),0) - ROUND((IFNULL(ROUND(sum(s.sheet)), 0) * (SELECT admin_sharing FROM userlogin a WHERE ${whereAid})) /100))as net_amount  FROM sheet s where s.isactive = 1 ${where} ORDER BY s.id desc`,
  ...
  getAllAdmin: (data, callBack) => {
    ...
    if (data.code != "") {
      // where += ` AND s.code = '${data.code}'`;
      whereUid += ` AND u.code = '${data.code}'`;
    }
    pool.query(
      `SELECT 
      u.id,u.parent_id,u.username,u.code,
      (SELECT IFNULL(sum(s.amount), 0) FROM salary s WHERE s.parent_id=u.id ${where})as total_salary,
        ROUND(((SELECT IFNULL(SUM(s.sheet),0) FROM sheet s WHERE s.parent_id = u.id ${where}) * u.admin_sharing)/100) as total_sheet,
        (ROUND(((SELECT IFNULL(SUM(s.sheet),0) FROM sheet s WHERE s.parent_id = u.id ${where}) * u.admin_sharing)/100) - (SELECT IFNULL(sum(s.amount), 0) FROM salary s WHERE s.parent_id=u.id ${where}))as final_amount
    FROM userlogin u 
    WHERE u.isactive = 1 ${whereUid} 
    GROUP BY u.id
    ORDER BY u.id DESC
    `,
    // pool.query(
    //   `select 
    //   u.username,u.admin_sharing,
    //     (SELECT IFNULL(SUM(s.amount),0) FROM salary s WHERE s.parent_id=u.id ${where})AS total_salary,
    //     (SELECT IFNULL(SUM(s.sheet),0) from sheet s WHERE s.parent_id=u.id ${where})AS total_sheet,
    //     IFNULL(((SELECT IFNULL(SUM(s.sheet),0) from sheet s WHERE s.parent_id=u.id ${where})*u.admin_sharing)/100,0) AS sheet_sharing
    // FROM userlogin u 
    // where 
    //   u.isactive = 1  
    //     ${whereUid} 
